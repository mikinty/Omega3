'''
Implementation of the neural network that is run on the Raspberry Pi 
and sent to the Arduino for display on Omega3. 

We use the pybrain library to make our neural network much 
easier to implement, and to use for gaining hidden layer data.

This program also plots in livetime the reference, input and output
data points, to give the user an idea of what progress the neural network
has in its training.

TODO: port constants into a CONSTANTS.py file.

NOTE: this program is currently implemented for two Arduino UNOs. 
      A future version of this program will only use one Arduino Mega.
'''
import random
import math
import time
...
# Arduino serial numbers for detection
...
learningRate = 10
numFeatures = 7
numTrain = 100 # amount of training data
numHiddenUnits1 = 3
numHiddenUnits2 = 3
...
def sendMatrix(m, layer, matrixNum):
    ...
    :param m: 8x8 bit matrix
    :param layer: which hidden layer to send info to
    :param matrixNum: which matrix to communicate to
    '''
...
def pesos_conexiones(n):
    '''
    Prints out hidden layer data from a neural network.

    :param n: pybrain neural network
    '''
    paramArr = {}
...
def generateData(N, funNumber):
    '''
    Generate bit data on 8x8 for the neural network to train on.

    :param N: number of training data points to give to the model
    :param funNumber: function number label
    '''
    inputArr = []
    outputArr = []
...
if __name__ == '__main__':
    ...
    num = min(int(input("What function would you like to train with? ")), 7)
